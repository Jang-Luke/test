...
public class MessagesDAO {
	...
	public int updateMsg(int msgNo, String inputMsg, String inputTime) throws Exception {
		String tmptime = Stream.of(Stream.of(inputTime.split("/")).collect(Collectors.joining()).split("-")).collect(Collectors.joining());
		String time = Stream.of(Stream.of(tmptime.split(" ")).collect(Collectors.joining()).split(":")).collect(Collectors.joining());
		if (!time.matches("^\\d*$")) {
			return 0;
		}
		if (!(time.length() == 10)) {
		...
		try (Connection con = getConnection();
				PreparedStatement ps = con
						.prepareStatement("UPDATE MESSAGES SET MESSAGE = ?, WRITE_DATE = ? WHERE MSG_NO = ?");) {
